...
export interface EndingResolverDeps {
  addModifier: (endingId: string, value: number, source: string) => void;
  applyTrustEndingWeights: (addModifier: (endingId: string, value: number, source: string) => void) => void;
  getLockedEnding: (candidates: string[]) => string | null;
  selectWeightedEnding: (candidates: string[]) => string | null;
  getAllEndingWeights: () => { endingId: string; weight: number; probability: number }[];
}
