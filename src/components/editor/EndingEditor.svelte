...
<div class="ending-editor">
  <section class="section">
    ...
    <p class="section-desc">管理所有游戏结局，包括好结局、坏结局的定义及其解锁条件</p>
  ...
  <div class="editor-layout">
    ...
    <main class="ending-detail">
      ...
      {:else}
        ...
        <section class="section">
          <h3 class="subsection-title">🔗 关联节点</h3>
          <p class="subsection-desc">以下剧情节点会触发此结局（在节点编辑器中设置 endingId）</p>

          {#if !endingNodeMap[ending.id]?.length}
            <div class="empty-mini">
              <p>暂无关联节点，请在节点编辑器中将节点的 endingId 设置为 "{ending.id}"</p>
            </div>
          {:else}
            <div class="linked-nodes">
              {#each endingNodeMap[ending.id] as nodeId}
                {@const node = nodes.find(n => n.id === nodeId)}
                <div class="linked-node-card" on:click={() => selectNode(nodeId)}>
                  <div class="ln-icon">📝</div>
                  <div class="ln-info">
                    <span class="ln-id">{nodeId}</span>
                    <span class="ln-title">{node?.title || '(无标题)'}</span>
                  </div>
                  <div class="ln-meta">
...
<style>
  ...
  .action-btn:hover {
    background: rgba(192, 144, 255, 0.28);
  }
  ...
  .form-item.checkbox-item-inline {
    grid-column: 1 / -1;
  }
  ...
  .input:focus {
    border-color: rgba(192, 144, 255, 0.5);
    background: rgba(192, 144, 255, 0.04);
  }
  ...
  .linked-node-card:hover {
    background: rgba(192, 144, 255, 0.1);
    border-color: rgba(192, 144, 255, 0.3);
  ...
  .preview-card.good {
    background: linear-gradient(135deg, rgba(100, 255, 150, 0.08), rgba(100, 255, 150, 0.02));
    border-color: rgba(100, 255, 150, 0.25);
  }
  ...
  .preview-card.good .preview-type {
    color: #64ff96;
  }

  .preview-card.bad .preview-type {
    color: #ff8080;
  }
